  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        ...
        const userData = {
          uid: currentUser.uid,
          displayName: currentUser.displayName || (currentUser.isAnonymous ? 'Guest User' : 'Anonymous'),
          email: currentUser.email || (currentUser.isAnonymous ? 'guest@crazygui.com' : 'none'),
          photoURL: currentUser.photoURL || `https://ui-avatars.com/api/?name=${encodeURIComponent(currentUser.displayName || 'Guest')}&background=random`,
          lastLogin: serverTimestamp(),
          isAnonymous: currentUser.isAnonymous
        };
        
        setDoc(userRef, userData, { merge: true }).catch(err => console.error("User sync error:", err));
