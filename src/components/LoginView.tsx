...
import { Mail, Github, Loader2, ArrowLeft, User as UserIcon, AlertCircle } from 'lucide-react';
import { auth, googleProvider, signInWithPopup, onAuthStateChanged, signInAnonymously, handleAuthError } from '../lib/firebase';
        ...
        <AnimatePresence>
          {errorMsg && (
            <motion.div 
              ...
              className="w-full mb-6 p-4 bg-red-400/10 border border-red-400/20 rounded-xl flex gap-3 text-red-400 text-xs font-bold"
            >
        ...
        <div className="w-full space-y-6">
          ...
          <div className="space-y-3">
            ...
            <button 
              ...
              disabled={!!loading}
              className="w-full h-12 bg-white/5 border border-white/10 hover:bg-white/10 text-white font-bold rounded-lg flex items-center justify-center gap-3 transition-all active:scale-[0.98] disabled:opacity-50 text-sm"
            >
              {loading === 'guest' ? (
                <Loader2 className="animate-spin" size={18} />
              ) : (
